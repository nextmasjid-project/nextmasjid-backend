...
namespace NextMasjid.Backend.Core.Data
{
    ...
    {
        public static void WriteScores(string path, IEnumerable<KeyValuePair<(int, int), int>> data)
        {
        ...
        public static void WriteScoreSegmented(string folderPath, string namePrefix, Dictionary<(int, int), int> data, int every = 8 * 1000 * 1000)
        {
            ...
            while(remaining > 0)
            {
                int chunk = remaining > every ? every : remaining;
                WriteScores(Path.Combine(folderPath, namePrefix + count.ToString() + ".dat"), data.Skip(data.Count - remaining).Take(chunk));
                count++;
                remaining -= chunk;
            }
        }

        public static Dictionary<(int, int), int> ReadScoresSegmented(string folderPath)
        {
